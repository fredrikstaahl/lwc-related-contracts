 ...
 */
 ...
 import info_text from '@salesforce/label/c.RELATED_CONTRACTS_TEXT';
 ...
 import column_no_of_prices from '@salesforce/label/c.RELATED_CONTRACTS_COLUMN_NUMBER_OF_PRICES';
...
export default class RelatedContractsCmp extends NavigationMixin(LightningElement) {
    @api recordId;
    @track contractList;
    @track contractData = [];
    @track selectedRow;
    ...
    columns = [
            {label: column_contract, fieldName: 'navUrl', type: 'url' , typeAttributes:{label: { fieldName: 'contractName' }}},
            {label: column_startdate, fieldName: 'startDate', type:'date-local'},
            {label: column_enddate, fieldName: 'endDate', type:'date-local'},
            {label: column_no_of_prices, fieldName:'numberOfContractedPrices', type:'number'}
        ];

    connectedCallback() {
        console.log('in renderedCallback');
        console.log('opportunityId = ' + this.recordId);
        getContracts({'recordId' : this.recordId, 'sObjectName' : this.sObjectName})
        .then(result => {
            ...
            this.contractList.forEach(contract => {
                ...
                console.log('contractPrices = ' + JSON.stringify(contract.NumberOfContractedPrices__c));
                const newContract = {
                    'contractName' :  contract.Account.Name,
                    'startDate' : contract.StartDate,
                    'endDate' : contract.EndDate,
                    'numberOfContractedPrices' : contract.NumberOfContractedPrices__c,
                    'Id' : contract.Id,
            ...
            if (!this.IsAccountPage) {
                getCurrentContract({'opportunityId' : this.recordId})
                            .then(result => {
                                console.log('result', JSON.stringify(result));
                                let rows = [result];
                                this.preSelectedRows = rows;
                                this.currentContractId = result;
                                console.log('setting preselectedrows=', JSON.stringify(this.preSelectedRows));
                                })
            }
    ...
    get submitButtonDisabled() {
        return (this.selectedRow !== undefined && this.selectedRow.Id === this.currentContractId) || this.selectedRow === undefined;
    }
